...
if [ "$1" = 'mysqld' -a -z "$wantHelp" ]; then
	...
	if [ ! -d "$DATADIR/mysql" ]; then
		...
		if [ -z "$SYNC_PORT" ] ;then
			SYNC_PORT=3306
			echo ########## set env SYNC_PORT is "$SYNC_PORT" ##########
		fi
		if [ -z "$SYNC_HOST" ] ;then
			SYNC_HOST=3306
			echo ########## set env SYNC_HOST is "$SYNC_HOST" ##########
		fi
		sync_sql="change master to master_host='$SYNC_HOST',master_port=$SYNC_PORT,master_user='sync',master_password='$SYNC_PASSWORD', master_log_file='mysql-bin.000001',master_log_pos=4;"
		echo "########## $sync_sql ##########"
		echo "$sync_sql" | "${mysql[@]}"
		echo "########## start slave; ##########"
